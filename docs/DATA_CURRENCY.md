...
| District population | 61 of 61 records have 2004 population. | U.S. Census Bureau tables distributed through HDX provide ADM2 census values from 2004 in the imported workbook. | Old, but acceptable because no newer reusable ADM2 source has been confirmed. | Keep clearly dated as historical and search for newer ADM2 data before release. |
...
| Administrative boundaries, hierarchy, areas, and centroids | Current seed uses geoBoundaries data derived from UN OCHA sources, with boundary year 2017, source update in 2023, and build date in 2023. | The direct HDX/OCHA administrative-boundary package has 2026 resource metadata and should be compared. | Acceptable seed, but not final enough for first release without comparison. | Compare current records against the direct HDX/OCHA package and refresh if it materially improves coverage or correctness. |
| Localities | 7,593 records are seeded from HDX/OCHA populated places and enriched with GeoNames where reviewed. No locality population is stored. | HDX/OCHA populated places has dataset date 2020-08-16, resource modified in 2022, and metadata modified in 2026. | Acceptable seed, but should stay on the freshness watchlist. | Recheck HDX/OCHA for newer populated-place resources before release. |
...
2. Compare the current geoBoundaries-derived ADM1, ADM2, and ADM3 records against the direct HDX/OCHA administrative-boundary package.
