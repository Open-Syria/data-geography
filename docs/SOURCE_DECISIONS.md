...
| `uscb-syria-population` | `approved` | Dated district and subdistrict population measurements from U.S. Census Bureau tables distributed through HDX. | May be used for dated population measurements when sentinel or missing values are not imported as real values. |
